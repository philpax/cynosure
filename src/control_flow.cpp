...
// jc/jb imm8
MAKE_OPCODE(72) { LOG_STREAM << PRINT_VALUE( (int32_t)NEXT_INS(1) ); if (EFLAGS.carry) { EIP.r += (int8_t)NEXT_INS(1); LOG_STREAM << std::endl; } }

// jz/je imm8
MAKE_OPCODE(74) { LOG_STREAM << PRINT_VALUE( (int32_t)NEXT_INS(1) ); if (EFLAGS.zero) { EIP.r += (int8_t)NEXT_INS(1); LOG_STREAM << std::endl; } }

// jnz/jne imm8
MAKE_OPCODE(75) { LOG_STREAM << PRINT_VALUE( (int32_t)NEXT_INS(1) ); if (!EFLAGS.zero) { EIP.r += (int8_t)NEXT_INS(1); LOG_STREAM << std::endl; } }
...
// jmp imm32
MAKE_OPCODE(E9) { if (ARG(1) == -5 || ARG(1) == -3) { state->running = false; } LOG_STREAM << PRINT_VALUE( (uint32_t)ARG(1) ); EIP.r += ARG(1); LOG_STREAM << std::endl; }